...
import torch
import math
from elytra.torch_utils import all_comb
...
def compute_v2v_dist(
    v3d_cam_gt: torch.Tensor, v3d_cam_pred: torch.Tensor, is_valid: torch.Tensor
):
    v2v = []
...
def compute_joint2d_error(joints2d_gt, joints2d_pred, valid_jts, img_res):
    ...
    valid_idx = valid_jts.long().nonzero().view(-1)
...
def compute_arti_error(pred_radian, gt_radian):
