...
def get_norm_smpl_mesh(gt_3d_joints, smpl_param, cam_param, smpl_model):
    ...
    reorg_h36m_from_smpl = (
        reorg_h36m_from_smpl
        - np.mean(reorg_h36m_from_smpl, 0)[None, :]
...
def add_Pose2Mesh_smpl_labels(
    ...
    # Check subject #1
    try:
        pose2mesh_3djoint_world_1 = np.array(
            joints[str(subject_id)][str(action_id)]["1"][str(frame_id)],
            dtype=np.float32,
        )
        pose2mesh_S24_1 = pose2mesh_joint_norm(pose2mesh_3djoint_world_1, R, t)
        matching_error_1 = np.sum(
            np.abs(gt_3d_joints[:13, :3] * 1000 - pose2mesh_S24_1[:13, :3])
        )
    ...
    # Check subject #2
    try:
        pose2mesh_3djoint_world_2 = np.array(
            joints[str(subject_id)][str(action_id)]["2"][str(frame_id)],
            dtype=np.float32,
        )
        pose2mesh_S24_2 = pose2mesh_joint_norm(pose2mesh_3djoint_world_2, R, t)
        matching_error_2 = np.sum(
            np.abs(gt_3d_joints[:13, :3] * 1000 - pose2mesh_S24_2[:13, :3])
        )
    ...
    if len(fail_subaction) == 2:
        ...
        pose2mesh_3djoint_world = set_pose2mesh_3djoint_world[
            np.argmin(set_matching_error)
        ]
    ...
    try:
        smpl_param = smpl_params[str(subject_id)][str(action_id)][str(subaction_id)][
            str(frame_id)
        ]
    ...
    if has_smpl == True and success_match == True:
        ...
        smpl_cam_pose, smpl_cam_shape, fitting_error24 = get_norm_smpl_mesh(
            gt_3d_joints, smpl_param, cam_param, smpl_mesh_model
        )
    else:
...
def preproc(mode, output_set):
    ...
    data_npz_path = (
        GRAPHCMR_dataset_path + "/h36m_" + mode + "_protocol" + str(protocol) + ".npz"
    )
    ...
    for img_i in tqdm(range(total_data_count)):
        ...
        if mode == "train":
            ...
            if (
                fitting_error24 > 50
            ):  # I empirically set it to 50, and it seems good enough
                has_smpl = False
        else:
            has_smpl = False
        ...
        has_2d_joints = False
        if mode == "train":
            gt_2d_joints_tag = pose2mesh_3d_to_2d_joint(
                pose2mesh_3djoint_world, R, t, f, c
            )
            has_2d_joints = True
        else:
