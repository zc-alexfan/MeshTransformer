...
def preproc(dataset_img_folder, dataset_tsv_folder, split):
    ...
    for img_i in tqdm(imgs):
        ...
        scaleFactor = 1.2
        scale = scaleFactor * max(bbox[2] - bbox[0], bbox[3] - bbox[1]) / 200.0
        ...
        # step 5. 3d pose
        gt_vertices = smpl(
            pose_tensor, betas_tensor
        )  # output shape: torch.Size([1, 6890, 3])
        gt_keypoints_3d = smpl.get_joints(
            gt_vertices
        )  # output shape: torch.Size([1, 24, 3])
