...
def run(args, train_dataloader, METRO_model, mano_model, renderer, mesh_sampler):
    ...
    optimizer = torch.optim.Adam(
        params=list(METRO_model.parameters()),
        lr=args.lr,
        betas=(0.9, 0.999),
        weight_decay=0,
    )
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        if iteration % args.logging_steps == 0 or iteration == max_iter:
            ...
            loss_dict = {
                "total_loss": log_losses.avg,
                "loss_2d": log_loss_2djoints.avg,
                "loss_3d": log_loss_3djoints.avg,
                "loss_v": log_loss_vertices.avg,
            }
            comet_utils.log_dict(
                experiment, loss_dict, step=iteration, postfix="_train"
            )
